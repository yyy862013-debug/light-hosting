...
import {
  Download, CheckCircle2, Terminal,
  Info, Archive, ChevronDown,
} from "lucide-react";
import {
  SOFTWARE_REGISTRY,
  SOFTWARE_GROUPS,
...
// ─────────────────────────────────────────────────────────────────────────────
// DownloadResolver
// Lets the user pick a software + version and preview the resolved
// download URL, file type, install pipeline, and any extraction warnings.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function DownloadResolver() {
  ...
  const handleResolve = () => {
    setLoading(true);
    setResult(null);
    // Simulate async resolution (replace with real API call in production)
    setTimeout(() => {
      setResult(resolveDownload(softwareId, version));
      setLoading(false);
    }, 600);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
        <div>
          ...
          <div style={{ position: "relative" }}>
            <select style={selectStyle} value={softwareId} onChange={handleSoftwareChange}>
              {SOFTWARE_GROUPS.map((g) => (
                <optgroup key={g.id} label={`${g.emoji} ${g.label}`}>
                  {g.ids.map((id) => (
                    <option key={id} value={id}>{SOFTWARE_REGISTRY[id]?.name}</option>
                  ))}
                </optgroup>
              ))}
            </select>
        ...
        <div>
          ...
          <div style={{ position: "relative" }}>
            <select style={selectStyle} value={version} onChange={(e) => { setVersion(e.target.value); setResult(null); }}>
      ...
      <button
        ...
        style={{
          ...
          background: loading ? "rgba(63,63,70,0.5)" : "linear-gradient(135deg,#10b981,#059669)",
      ...
      >
        {loading
          ...
          : <><Download style={{ width: "16px", height: "16px" }} />Resolve Download URL</>
        }
      </button>

      {/* Result card */}
      {result && !result.error && (
        <div style={{ background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.25)", borderRadius: "16px", padding: "20px", display: "flex", flexDirection: "column", gap: "14px", animation: "fadeIn 0.3s ease-out" }}>

          {/* Title row */}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <CheckCircle2 style={{ width: "16px", height: "16px", color: "#10b981" }} />
              <span style={{ fontSize: "13px", fontWeight: 600, color: "#6ee7b7" }}>URL Resolved</span>
            </div>
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
              <Badge text={result.badge}                 color={result.color} />
              <Badge text={result.fileType.toUpperCase()} color={result.isZipExtract ? "#f97316" : "#6366f1"} />
              {result.isModpack && <Badge text="MODPACK" color="#f97316" />}
            </div>
          </div>
          ...
          <div style={{ background: "#0a0a0f", border: "1px solid #27272a", borderRadius: "10px", padding: "12px 14px", display: "flex", alignItems: "flex-start", gap: "10px" }}>
            ...
            <span style={{ fontSize: "11px", color: "#a1a1aa", wordBreak: "break-all", flex: 1, fontFamily: "monospace", lineHeight: "1.6" }}>
              {result.downloadUrl}
            </span>
            <button
              onClick={handleCopy}
              style={{ background: copied ? "rgba(16,185,129,0.2)" : "rgba(63,63,70,0.5)", border: "none", color: copied ? "#6ee7b7" : "#a1a1aa", fontSize: "11px", fontWeight: 600, padding: "4px 10px", borderRadius: "6px", cursor: "pointer", flexShrink: 0, whiteSpace: "nowrap" }}
            >
              {copied ? "Copied!" : "Copy"}
            </button>
          </div>
          ...
          {/* Install pipeline */}
          <div>
            <div style={{ fontSize: "11px", color: "#71717a", fontWeight: 600, marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.5px" }}>
              Install Pipeline
            </div>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "flex-start", background: "rgba(249,115,22,0.08)", border: "1px solid rgba(249,115,22,0.3)", borderRadius: "10px", padding: "12px" }}>
              ...
              <div>
                <div style={{ fontSize: "12px", fontWeight: 600, color: "#fb923c", marginBottom: "4px" }}>
                  {result.isModpack ? "Modpack Extraction Required" : "Archive Extraction Required"}
                </div>
                <div style={{ fontSize: "11px", color: "#a1a1aa", lineHeight: "1.6" }}>
                  {result.isModpack
                    ? "Backend will extract the archive, parse the manifest, then download each mod JAR and config file individually."
                    : "Backend will extract the archive and set executable permissions on the server binary."}
                </div>
              </div>
            </div>
          )}
